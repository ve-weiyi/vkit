...
type Option func(*CaptchaStore)

// WithStore 使用统一的 storex.KVStore
func WithStore(store storex.KVStore, expiration time.Duration) Option {
	return func(o *CaptchaStore) {
		o.store = newStoreAdapter(store, expiration)
	}
}

func NewCaptchaStore(options ...Option) *CaptchaStore {
	ch := &CaptchaStore{
...
// 生成随机验证码
func (rs *CaptchaStore) GetCodeCaptcha(key string) (code string, err error) {
...
func (rs *CaptchaStore) GetMathImageCaptcha(height int, width int) (string, string, string, error) {
	if height == 0 {
...
func (rs *CaptchaStore) GetImageCaptcha(CaptchaType string, height int, width int, length int) (string, string, error) {
...
// 验证验证码
func (rs *CaptchaStore) VerifyCaptcha(id string, answer string) bool {
