...
type StorageConfig struct {
	Provider  string `json:"provider"`   // 服务商类型：aliyun | tencent | qiniu | local
	Endpoint  string `json:"endpoint"`   // 服务端点（OSS/COS endpoint）
	Bucket    string `json:"bucket"`     // 存储桶名称
	AccessKey string `json:"access_key"` // AccessKey / SecretId
	SecretKey string `json:"secret_key"` // SecretKey
	Region    string `json:"region"`     // 地域
	CDNDomain string `json:"cdn_domain"` // CDN域名（可选）
	BasePath  string `json:"base_path"`  // 文件存储基础路径（可选）
	IsPrivate bool   `json:"is_private"` // 是否私有存储
}
