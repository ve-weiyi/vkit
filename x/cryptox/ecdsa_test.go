...
func Test_(t *testing.T) {
	...
	// 打印结果
	printKey("Private_Key", privateKeyBytes)
	printKey("Public_Key", publicKeyBytes)
	printKey("hash", hash[:])
	printKey("Signature", signature)
...
func printKey(name string, data []byte) {
	printBytes(name, data)
	printHexBytes(name, data)
	printBase64Bytes(name, data)
}

// printBytes 打印字节数组为C风格的数组定义
func printBytes(name string, data []byte) {
	fmt.Println(name, "uint8_t len(", len(data), ") :", data)
...
func printHexBytes(name string, data []byte) {
	...
	fmt.Println(name, "hex len(", len(hexStr), ") :", hexStr)
}

func printBase64Bytes(name string, data []byte) {
	base64Str := base64.StdEncoding.EncodeToString(data)
	fmt.Println(name, "base64 len(", len(base64Str), ") :", base64Str)
}
