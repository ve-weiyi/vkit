// 诊断程序：用 gorm.io/gen 从真实 MySQL 反向生成 GORM 模型，
// 并可查看库表结构、执行 AutoMigrate 对照。
// 需要真实数据库凭证，故不放测试目录，由人工手动执行，非正式产物。
//
//	TEST_MYSQL_DSN='root:pwd@(127.0.0.1:3306)/blog?charset=utf8mb4&parseTime=True&loc=Local' \
//	  go run ./cmd/gormgen -mode=generate -table=t_user -out=./runtime/gormgen
package main
...
func main() {
	mode := flag.String("mode", "generate", "执行模式：generate（生成模型）| inspect（查看表结构）| migrate（AutoMigrate）")
	table := flag.String("table", "t_user", "要生成模型的表名（generate 模式用）")
	out := flag.String("out", "./runtime/gormgen", "生成目录（generate 模式用）")
	flag.Parse()

	dsn := os.Getenv("TEST_MYSQL_DSN")
	if dsn == "" {
		fatalf("请通过环境变量 TEST_MYSQL_DSN 提供数据库连接串")
	}
	...
	switch *mode {
	case "generate":
		if err := generate(db, *table, *out); err != nil {
			fatalf("生成模型失败: %v", err)
		}
	case "inspect":
		inspect(db)
	...
	default:
		fatalf("未知的 -mode: %s（可选 generate | inspect | migrate）", *mode)
	}
}

// openDB 按 DSN 连接 MySQL，表名使用单数形式。
...
// generate 用 gen 把指定表反向生成为 GORM 模型与查询代码。
func generate(db *gorm.DB, table string, out string) error {
	// 生成实例
	...
	g.ApplyBasic(g.GenerateModel(table, fieldOpts...))
	g.Execute()
	return nil
}

// inspect 打印库名、各表类型、列与索引，便于核对真实库结构。
func inspect(db *gorm.DB) {
	...
	for _, table := range tables {
		...
		fmt.Println("table-->", debugx.Dump(tableType))

		columns, err := db.Migrator().ColumnTypes(table)
		if err != nil {
			log.Println("-->", err)
		}
		fmt.Println("columns-->", debugx.Dump(columns))

		indexes, err := db.Migrator().GetIndexes(table)
		if err != nil {
			log.Println("-->", err)
		}
		fmt.Println("indexes-->", debugx.Dump(indexes))
	}
}
